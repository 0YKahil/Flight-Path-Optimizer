...
class Graph {
    public:
         ...
         */
        std::pair <std::vector<int>, int> findShortestPath(const Airport& start, const Airport& destination);

        // THIS VERSION DOES THE SAME AS ABOVE, BUT WILL ALWAYS RECOMMEND THE SHORTEST ROUTE WITH A VERY LOW AMOUNT OF LANDINGS
        std::pair <std::vector<int>, int> findShortestPathMIN(const Airport& start, const Airport& destination);
        /**
